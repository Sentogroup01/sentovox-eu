---
type: "about"
slug: "over"
title: "Over SentoVox - Europees bezoekersonderzoek voor podiumkunsten sinds 2024"
description: "Over SentoVox - Europees publieksonderzoeksprogramma voor theaters, concertzalen en cultuurhuizen. 130+ zalen, 4 universiteiten, wetenschappelijk onderbouwd."
keywords: "over SentoVox, podiumkunsten onderzoek, Europese theaterstudie, publieksonderzoek geschiedenis"
kicker: "De Europese publieksstudie voor podiumkunsten"
intro: "SentoVox is een wetenschappelijk onderbouwd bezoekersonderzoeksprogramma voor theaters, concertzalen en cultuurhuizen in heel Europa. Gebouwd met 4 universiteiten en sector-partners."

stats:
  - value: "130+"
    label: "Cultuurhuizen in Europa"
  - value: "4"
    label: "Universitaire partners"
  - value: "6"
    label: "Talen"
  - value: "60.000+"
    label: "Bezoekersreacties"

history_title: "Geschiedenis & uitbreidingsroute"
history_body: |
  Het project ging in **2024** van start in **België** met een pilot bij cultuurcentra uit het Cult-netwerk, in nauwe samenwerking met de **Universiteit Antwerpen**. De pilot bevestigde de methodologie en groeide uit tot een vaste deelnemersgroep van 100+ Vlaamse en Brusselse zalen.

  In **2025** breidde SentoVox uit naar **Nederland** in samenwerking met VSCD-theaters, met deelname van zalen zoals Theater aan het Vrijthof, Parktheater Eindhoven, Concertgebouw de Doelen, Schouwburg Hengelo en Stadstheater Zoetermeer. In **2025-2026** volgde uitbreiding naar **Duitsland** (Bühnenverein-partners) en **Oostenrijk**, met onderzoekspartners aan HAWK Hildesheim en FH Kufstein Tirol.

  In **2026** wordt SentoVox uitgerold in **Frankrijk, Italië, Spanje en Zwitserland** en daarmee een echt pan-Europees instrument. Vandaag is het platform actief in 6 talen, met 130+ deelnemende zalen en partner-universiteiten in 7 landen.

report_kicker: "Europese benchmark"
report_title: "Europees rapport van SentoVox"
report_body: "Ontdek hoe cultuurhuizen SentoVox gebruiken om bezoekers, programmakwaliteit en maatschappelijke impact te begrijpen."
report_btn: "Open het laatste rapport (PDF)"
report_url: "#"

testimonials_kicker: "Wat zalen zeggen"
testimonials_title: "Hoe cultuurhuizen SentoVox waarderen"
testimonials:
  - quote: "Het onderzoek werd zeer goed ontvangen. We kregen concrete aanbevelingen en de Europese benchmark helpt onze programmering te verfijnen."
    name: "Directeur"
    role: "Cultuurcentrum Brugge, België"
  - quote: "We krijgen duidelijke aanknopingspunten. Dashboard en visualisaties zijn direct bruikbaar voor onze stuurgroep."
    name: "Hoofd publieksonderzoek"
    role: "Concertgebouw de Doelen, Nederland"
  - quote: "Het Impact Compass Model bracht zaken aan het licht die we hadden gemist."
    name: "Communicatie"
    role: "Parktheater Eindhoven, Nederland"
  - quote: "Dank voor het rapport - een schat aan informatie waar we direct mee aan de slag gaan."
    name: "Directie"
    role: "Opéra de Lyon, Frankrijk"

universities_kicker: "Academische partners"
universities_title: "Wetenschappelijke basis"
universities:
  - name: "Universiteit Antwerpen"
  - name: "Université de Bordeaux"
  - name: "Universitat de València"
  - name: "Universität Hildesheim"
  - name: "Rijksuniversiteit Groningen"
  - name: "Ca' Foscari University of Venice"
  - name: "FH Kufstein Tirol"
# build: 2026-04-29
---
